...
with open("./datasets/files/uf-languages-with-radio-broadcast.html") as fp:
    ...
    for row in rows:
        ...
        if cols:
            print(
                f"Language: {cols[1]}\nMedia: {cols[2]}\nPublished: {cols[3]}\nInfo: {cols[4]}\n\n\n")
            # break
            cur.execute("insert into uf_languages_with_radio_broadcast (language, media, published, info) VALUES (%s, %s, %s, %s)",
